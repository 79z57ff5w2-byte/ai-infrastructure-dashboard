...
df = pd.DataFrame(HOLDINGS_DATA, columns=["ETF","Ticker","Company","Weight"])
...
df["Layer"] = df.apply(lambda r: classify_layer(r["Company"], r["Ticker"]), axis=1)
...
def validate_signals(signals):
    ...
    validation["Actionability Score"] = validation["Signal Score"]
    validation.loc[validation["Signal Category"] == "Noise", "Actionability Score"] = validation["Actionability Score"] * 0.3
    validation.loc[validation["Signal Category"] == "Tactical", "Actionability Score"] = validation["Actionability Score"] * 0.7
