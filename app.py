...
st.set_page_config(
    page_title="AI Portfolio Decision Engine + Signal Filtering + Backtest",
    page_icon="⚡",
    layout="wide"
)
...
# -----------------------------
# Classification and signal filtering
# -----------------------------
...
def classify_signal_layer(title):
    return classify_holding(title, "")
...
def raw_signal_score(title, summary=""):
    ...
    high_keywords = [
        "capex", "capital expenditure", "data center", "datacenter", "power", "nuclear",
        "gpu", "chip", "semiconductor", "supply", "shortage", "contract", "deal",
        "partnership", "expansion", "demand", "electricity", "grid", "cloud",
        "ai infrastructure", "hpc"
    ]

    very_high_keywords = [
        "billion", "$", "megawatt", "mw", "gigawatt", "gw", "multi-year",
        "nvidia", "tsmc", "microsoft", "amazon", "alphabet", "constellation",
        "coreweave", "hyperscaler"
    ]

    bearish_keywords = [
        "delay", "cancel", "slowdown", "shortfall", "restriction", "export control",
        "supply constraint", "margin pressure", "overcapacity"
    ]
...
def signal_category(title, summary=""):
    ...
    structural_keywords = [
        "multi-year", "billion", "$", "capex", "capital expenditure", "power purchase",
        "ppa", "nuclear", "grid", "megawatt", "gigawatt", "mw", "gw", "data center",
        "datacenter", "contract", "deal", "supply agreement", "partnership", "buildout",
        "expansion", "capacity", "plant", "facility"
    ]

    tactical_keywords = [
        "earnings", "guidance", "forecast", "quarter", "sales", "revenue", "margin",
        "demand", "launch", "product", "analyst", "upgrade", "downgrade"
    ]

    noise_keywords = [
        "could", "may", "might", "opinion", "rumor", "rumour", "hype", "says",
        "why", "how", "what", "top", "best", "stock to watch"
    ]
...
def signal_market_read(title, layer, category):
    ...
    if layer == "Energy":
        return "AI demand may be moving from compute scarcity to power scarcity."
    if layer == "Data Centers":
        return "Physical infrastructure and cooling capacity may be becoming more valuable."
    if layer == "Compute":
        return "Chip and accelerator demand remains the core near-term AI capex driver."
    if layer == "Data/Models":
        return "Model scaling and enterprise data demand may support the data/model layer."
    if layer == "Platforms/Apps":
        return "Platform winners may be converting infrastructure spend into user-facing revenue."
    if layer == "Crypto AI Pivot":
        return "Crypto miners may gain AI/HPC optionality where power and sites are valuable."
    return "General AI ecosystem signal."
    ...
    for layer, queries in SIGNAL_QUERIES.items():
        for query in queries[:2]:
            for item in fetch_google_news_rss(query, max_items=3):
                item_layer = classify_signal_layer(item["Title"])
...
def company_importance_rank(company, ticker, layer, signal_boosts=None):
    ...
    boosts = {
        ...
        "microsoft": 19, "amazon": 17, "alphabet": 17,
        "constellation": 18, "nextera": 15, "equinix": 16,
        "digital realty": 15, "vertiv": 20, "cameco": 13,
        "ge vernova": 14, "palantir": 12, "snowflake": 10,
        "meta": 12, "coinbase": 8, "strategy": 7,
        "bitmine": 7, "iren": 8, "hut 8": 7,
        "cleanspark": 7, "riot": 7, "marathon": 7, "mara": 7
    }
...
def build_company_gap_engine(overlap_df, signal_boosts=None):
    ...
    critical_companies = pd.DataFrame([
        ["NVDA", "NVIDIA", "Compute"], ["TSM", "TSMC", "Compute"],
        ["ASML", "ASML", "Compute"], ["AVGO", "Broadcom", "Compute"],
        ["AMD", "AMD", "Compute"], ["AMAT", "Applied Materials", "Compute"],
    ...
    gap["Dynamic Importance Score"] = gap.apply(
        lambda r: company_importance_rank(r["Company"], r["Holding Ticker"], r["AI Layer"], signal_boosts),
        axis=1
    )
...
def simulate_monthly_returns(months, seed=7):
    random.seed(seed)
    ...
    for month in range(1, months + 1):
        regime_boost = 0.0
        # crude AI cycle regime: later months favour infrastructure/AI tilts
        ...
        for etf in base_monthly:
            noise = random.gauss(0, vol[etf])
            extra = regime_boost if etf in ["AINF", "SEMI", "GXAI"] else 0
            crypto_extra = regime_boost * 1.2 if etf == "CRYP" else 0
            rows.append({
                "Month": month,
                "ETF": etf,
                "Return": base_monthly[etf] + extra + crypto_extra + noise
            })

    return pd.DataFrame(rows)

def run_backtest(start_value, monthly_contribution, months, static_weights, decision_weights, seed=7):
    returns = simulate_monthly_returns(months, seed=seed)

    static_value = start_value
    decision_value = start_value
    ...
    for month in range(1, months + 1):
        ...
        static_r = sum((static_weights.get(etf, 0) / 100) * month_returns.get(etf, 0) for etf in static_weights)
        decision_r = sum((decision_weights.get(etf, 0) / 100) * month_returns.get(etf, 0) for etf in decision_weights)

        static_value = static_value * (1 + static_r) + monthly_contribution
        decision_value = decision_value * (1 + decision_r) + monthly_contribution

        static_series.append({"Month": month, "Strategy": "Static baseline", "Portfolio Value": static_value})
        decision_series.append({"Month": month, "Strategy": "Decision engine tilt", "Portfolio Value": decision_value})

    result = pd.DataFrame(static_series + decision_series)
    return result
...
starter_holdings = pd.DataFrame([
    ["IVV", "MSFT", "Microsoft", 7.0], ["IVV", "NVDA", "NVIDIA", 6.5],
    ["IVV", "AAPL", "Apple", 6.0], ["IVV", "AMZN", "Amazon", 4.0],
    ["IVV", "GOOGL", "Alphabet", 3.5], ["IVV", "META", "Meta Platforms", 2.5],
    ["IVV", "AVGO", "Broadcom", 2.0], ["IVV", "TSLA", "Tesla", 1.5],
    ["IVV", "JPM", "JPMorgan Chase", 1.3], ["IVV", "XOM", "Exxon Mobil", 1.2],

    ["VEU", "ASML", "ASML", 1.5], ["VEU", "TSM", "TSMC", 1.4],
    ["VEU", "SAP", "SAP", 1.0], ["VEU", "NESN", "Nestle", 1.0],
    ["VEU", "NOVO", "Novo Nordisk", 0.9], ["VEU", "TM", "Toyota Motor", 0.8],
    ["VEU", "SHEL", "Shell", 0.7], ["VEU", "AZN", "AstraZeneca", 0.7],
    ["VEU", "TCEHY", "Tencent", 0.7], ["VEU", "BHP", "BHP Group", 0.6],

    ["NDQ", "NVDA", "NVIDIA", 8.5], ["NDQ", "MSFT", "Microsoft", 7.8],
    ["NDQ", "AAPL", "Apple", 7.5], ["NDQ", "AMZN", "Amazon", 5.5],
    ["NDQ", "AVGO", "Broadcom", 4.8], ["NDQ", "META", "Meta Platforms", 4.6],
    ["NDQ", "GOOGL", "Alphabet", 4.5], ["NDQ", "TSLA", "Tesla", 3.0],
    ["NDQ", "COST", "Costco", 2.5], ["NDQ", "NFLX", "Netflix", 2.3],

    ["VHY", "BHP", "BHP Group", 10.3], ["VHY", "CBA", "Commonwealth Bank of Australia", 9.8],
    ["VHY", "NAB", "National Australia Bank", 6.8], ["VHY", "WDS", "Woodside Energy", 6.7],
    ["VHY", "WBC", "Westpac Banking", 6.0], ["VHY", "ANZ", "ANZ Group", 5.5],
    ["VHY", "MQG", "Macquarie Group", 3.5], ["VHY", "RIO", "Rio Tinto", 3.2],
    ["VHY", "TLS", "Telstra", 2.8], ["VHY", "WOW", "Woolworths", 2.5],

    ["AINF", "GEV", "GE Vernova", 5.9], ["AINF", "CCO", "Cameco Corporation", 5.4],
    ["AINF", "SCCO", "Southern Copper", 5.4], ["AINF", "FCX", "Freeport-McMoRan", 5.0],
    ["AINF", "VRT", "Vertiv", 4.8], ["AINF", "ETN", "Eaton", 4.5],
    ["AINF", "EQIX", "Equinix", 4.0], ["AINF", "DLR", "Digital Realty", 3.8],
    ["AINF", "CEG", "Constellation Energy", 3.6], ["AINF", "NEE", "NextEra Energy", 3.3],

    ["SEMI", "NVDA", "NVIDIA", 12.0], ["SEMI", "TSM", "TSMC", 10.0],
    ["SEMI", "ASML", "ASML", 9.0], ["SEMI", "AVGO", "Broadcom", 8.0],
    ["SEMI", "AMD", "AMD", 6.0], ["SEMI", "AMAT", "Applied Materials", 5.0],
    ["SEMI", "LRCX", "Lam Research", 4.5], ["SEMI", "MU", "Micron Technology", 4.0],
    ["SEMI", "QCOM", "Qualcomm", 3.5], ["SEMI", "INTC", "Intel", 3.0],

    ["CRYP", "BMNR", "BitMine Immersion Technologies", 10.4],
    ["CRYP", "MSTR", "Strategy Inc", 9.8], ["CRYP", "IREN", "IREN Ltd", 9.7],
    ["CRYP", "HUT", "Hut 8 Corp", 5.1], ["CRYP", "COIN", "Coinbase Global", 5.0],
    ["CRYP", "CLSK", "CleanSpark", 4.5], ["CRYP", "RIOT", "Riot Platforms", 4.0],
    ["CRYP", "MARA", "MARA Holdings", 4.0], ["CRYP", "CIFR", "Cipher Mining", 3.6],
    ["CRYP", "GLXY", "Galaxy Digital", 3.2],

    ["GXAI", "NVDA", "NVIDIA", 8.0], ["GXAI", "MSFT", "Microsoft", 7.0],
    ["GXAI", "GOOGL", "Alphabet", 6.0], ["GXAI", "META", "Meta Platforms", 5.0],
    ["GXAI", "PLTR", "Palantir Technologies", 4.5], ["GXAI", "SNOW", "Snowflake", 4.0],
    ["GXAI", "ADBE", "Adobe", 3.5], ["GXAI", "NOW", "ServiceNow", 3.0],
    ["GXAI", "CRM", "Salesforce", 3.0], ["GXAI", "AMZN", "Amazon", 3.0],
...
st.caption("Signal filtering + backtesting + holdings overlap + company gaps + ETF optimizer.")
...
with st.sidebar:
    ...
    default_weights = {
        "IVV": 22.0, "VEU": 22.0, "NDQ": 22.0, "VHY": 24.0,
        "AINF": 4.0, "SEMI": 4.0, "CRYP": 2.0, "GXAI": 0.0
    }
...
with tabs[0]:
    ...
    st.write("Separates structural signals from tactical signals and noise before they affect the decision engine.")

    if not use_live_signals:
        st.info("Live signals are disabled in the sidebar.")
    elif signals.empty:
        st.warning("No live signals could be fetched. The app is still running on the base importance model.")
    else:
        ...
        st.markdown("### Structural Signals — Act / Consider")
        structural = signals[signals["Signal Category"] == "Structural"].sort_values("Signal Score", ascending=False)
        st.dataframe(
            structural[["AI Layer", "Signal Score", "Sentiment", "Title", "Source", "Published", "Market Read", "Link"]],
            use_container_width=True,
            hide_index=True
        )

        st.markdown("### Tactical Signals — Watch")
        tactical = signals[signals["Signal Category"] == "Tactical"].sort_values("Signal Score", ascending=False)
        st.dataframe(
            tactical[["AI Layer", "Signal Score", "Sentiment", "Title", "Source", "Published", "Market Read", "Link"]],
            use_container_width=True,
            hide_index=True
        )

        st.markdown("### Noise — Ignore")
        noise = signals[signals["Signal Category"] == "Noise"].sort_values("Signal Score", ascending=False)
        st.dataframe(
            noise[["AI Layer", "Signal Score", "Sentiment", "Title", "Source", "Published", "Market Read", "Link"]],
            use_container_width=True,
            hide_index=True
        )
...
with tabs[1]:
    ...
    if not decision_plan.empty:
        ...
        st.success(f"Top action: {top_action['Action']} — {top_action['Company']} via {top_action['ETF Route']}")
...
with tabs[2]:
    ...
    st.write("This is a simplified scenario backtest. It compares a static portfolio against a signal-aware decision-engine tilt.")
    ...
    st.markdown("### Static baseline weights")
    static_weights = {
        "IVV": 22, "VEU": 22, "NDQ": 22, "VHY": 24,
        "AINF": 4, "SEMI": 4, "CRYP": 2, "GXAI": 0
    }

    st.markdown("### Decision-engine tilt weights")
    if target_mode == "AI Infrastructure Tilt":
        decision_weights = {"IVV": 20, "VEU": 24, "NDQ": 18, "VHY": 20, "AINF": 10, "SEMI": 5, "CRYP": 2, "GXAI": 1}
    elif target_mode == "Semiconductor Heavy":
        decision_weights = {"IVV": 20, "VEU": 22, "NDQ": 20, "VHY": 18, "AINF": 5, "SEMI": 12, "CRYP": 2, "GXAI": 1}
    elif target_mode == "Crypto AI Pivot":
        decision_weights = {"IVV": 20, "VEU": 22, "NDQ": 20, "VHY": 20, "AINF": 6, "SEMI": 5, "CRYP": 5, "GXAI": 2}
    else:
        decision_weights = {"IVV": 22, "VEU": 24, "NDQ": 20, "VHY": 20, "AINF": 7, "SEMI": 5, "CRYP": 1, "GXAI": 1}

    st.write("Static:", static_weights)
    st.write("Decision tilt:", decision_weights)

    bt = run_backtest(start_value, monthly_contribution, months, static_weights, decision_weights)
    ...
    static_final = float(final[final["Strategy"] == "Static baseline"]["Portfolio Value"].iloc[0])
    decision_final = float(final[final["Strategy"] == "Decision engine tilt"]["Portfolio Value"].iloc[0])
    delta = decision_final - static_final

    if delta > 0:
        st.success(f"In this simplified scenario, the decision-engine tilt ends ${delta:,.0f} ahead.")
    else:
        st.warning(f"In this simplified scenario, the decision-engine tilt ends ${abs(delta):,.0f} behind.")

    st.caption("This is a synthetic scenario test, not true historical performance. It is useful for stress-testing logic, not proving future returns.")
...
with tabs[3]:
    ...
    st.dataframe(
        ranking[["Holding Ticker", "Company", "AI Layer", "Dynamic Importance Score", "Portfolio_Exposure", "ETF_Count", "ETFs", "Company-Level Buy Signal"]],
        use_container_width=True,
        hide_index=True
    )
...
with tabs[4]:
    ...
    st.dataframe(
        gap_view[["Holding Ticker", "Company", "AI Layer", "Dynamic Importance Score", "Portfolio_Exposure", "Gap Score", "Company-Level Buy Signal", "ETF Route"]],
        use_container_width=True,
        hide_index=True
    )
...
with tabs[5]:
    ...
    st.dataframe(
        overlap[["Holding Ticker", "Company", "AI Layer", "Portfolio_Exposure", "ETF_Count", "ETFs", "Overlap Flag"]],
        use_container_width=True,
        hide_index=True
    )
...
- **Structural**: capex, power contracts, multi-year agreements, data-center expansion, grid constraints.
- **Tactical**: earnings, guidance, revenue, product launches, demand commentary.
- **Noise**: opinion, hype, vague headlines, weak/low-actionability items.

Only structural signals meaningfully boost the decision engine. Tactical signals can be toggled on/off in the sidebar.

### Backtesting

The backtest is a synthetic scenario test. It compares:
- a static baseline portfolio
- a decision-engine tilt portfolio

It is not true historical backtesting and does not predict future returns. It is used to stress-test whether the strategy logic behaves sensibly across scenarios.
