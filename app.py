...
PROVIDER_URLS = {
    "NDQ": "https://files.betashares.com.au/files/NDQ/holdings.csv",
    "SEMI": "https://files.betashares.com.au/files/SEMI/holdings.csv",
    "CRYP": "https://files.betashares.com.au/files/CRYP/holdings.csv",
}

def fetch_etf_holdings(etf):
    url = PROVIDER_URLS.get(etf)
    if not url:
        return None
...
def auto_refresh_holdings():
    frames = []
    for etf in PROVIDER_URLS:
        raw = fetch_etf_holdings(etf)
        if raw is None:
            continue
        cleaned, _ = clean_uploaded_holdings(raw, assigned_etf=etf)
...
def detect_changes(old_df, new_df):
    ...
    merged = new_df.merge(old_df, on=["ETF","Ticker"], how="left", suffixes=("_new","_old"))

    merged["Weight_old"] = merged["Weight_old"].fillna(0)
    merged["Change"] = merged["Weight_new"] - merged["Weight_old"]

    changes = merged[merged["Change"].abs() > 0.5]  # threshold

    return changes[["ETF","Ticker","Weight_old","Weight_new","Change"]]

# Auto refresh trigger (daily)
if "last_refresh" not in st.session_state:
    st.session_state["last_refresh"] = None

today = dt.date.today()

if st.session_state["last_refresh"] != today:
    new_data = auto_refresh_holdings()

    if new_data is not None:
        old_data = st.session_state.get("active_holdings_df")
        changes = detect_changes(old_data, new_data)

        st.session_state["active_holdings_df"] = new_data
        st.session_state["holdings_source"] = "Auto-refreshed ETF holdings"
        st.session_state["last_refresh"] = today
        st.session_state["holdings_changes"] = changes
...
def build_execution_plan(etf_demand, contribution_amount, min_trade_size=500, max_etfs=3):
    if etf_demand is None or etf_demand.empty:
        return pd.DataFrame(columns=[
            "Priority", "ETF", "Demand Score", "Suggested $", "Reason"
        ])

    work = etf_demand.copy()
    work = work.sort_values("Demand Score", ascending=False).head(max_etfs)

    total_demand = work["Demand Score"].sum()

    if total_demand <= 0 or contribution_amount <= 0:
        work["Suggested $"] = 0
    else:
        work["Suggested $"] = (work["Demand Score"] / total_demand * contribution_amount).round(0)

    # Apply minimum trade size by filtering small allocations, then re-normalising.
    work = work[work["Suggested $"] >= min_trade_size].copy()

    if work.empty:
        # If everything is below min trade size, put the whole amount into the top ETF.
        top = etf_demand.sort_values("Demand Score", ascending=False).head(1).copy()
        top["Suggested $"] = contribution_amount
        work = top

    else:
        total_after_filter = work["Suggested $"].sum()
        if total_after_filter > 0:
            work["Suggested $"] = (work["Suggested $"] / total_after_filter * contribution_amount).round(0)

    work["Priority"] = range(1, len(work) + 1)

    if "Top Drivers" in work.columns:
        work["Reason"] = "Driven by: " + work["Top Drivers"].astype(str)
    else:
        work["Reason"] = "Highest ETF demand score"

    return work[["Priority", "ETF", "Demand Score", "Suggested $", "Reason"]]

def build_commsec_ready_plan(execution_plan, brokerage_buffer=0):
    if execution_plan is None or execution_plan.empty:
        return pd.DataFrame(columns=["ETF", "Order Type", "Indicative Amount", "Note"])

    plan = execution_plan.copy()
    plan["Indicative Amount"] = (plan["Suggested $"] - brokerage_buffer).clip(lower=0)
    plan["Order Type"] = "Buy"
    plan["Note"] = "Use limit order or marketable limit; verify live price and spread before placing trade."

    return plan[["ETF", "Order Type", "Indicative Amount", "Note"]]

def execution_summary_text(execution_plan):
    if execution_plan is None or execution_plan.empty:
        return "No execution plan today. Hold cash until stronger ETF demand appears."

    top = execution_plan.iloc[0]
    return f"Primary ETF: {top['ETF']} — suggested allocation ${top['Suggested $']:,.0f}."
...
st.markdown("### 🔄 Holdings Change Detection")

changes = st.session_state.get("holdings_changes")

if changes is None or changes.empty:
    st.success("No significant holdings changes detected")
else:
    st.warning("Recent holdings changes detected")
...
with tabs[10]:
    ...
    st.write("Converts ETF demand into an exact ETF-only buying plan for your next capital deployment.")

    preset = st.selectbox(
        "Choose execution preset",
        list(LAYER_PRESETS.keys()),
        key="execution_preset"
    )
    ...
    min_trade_size = st.number_input(
        ...
        value=500,
        step=100,
        key="execution_min_trade"
    )

    max_etfs = st.slider(
        "Maximum ETFs to buy this cycle",
        min_value=1,
        max_value=5,
        value=2,
        key="execution_max_etfs"
    )

    brokerage_buffer = st.number_input(
        "Brokerage / cash buffer per trade ($)",
        min_value=0,
        value=0,
        step=5,
        key="execution_brokerage_buffer"
    )

    norm = get_weights("execution")
    ranking = build_dynamic_ranking(norm, preset, live_boosts=live_boosts)
    etf_demand = aggregate_etf_demand_from_ranking(ranking, norm)

    st.markdown("### ETF Demand Feeding Execution")
    if etf_demand.empty:
        st.info("No actionable ETF demand detected.")
    else:
        st.dataframe(etf_demand, use_container_width=True, hide_index=True)
    ...
    if execution_plan.empty:
        st.info("No trades suggested today.")
    else:
        st.dataframe(execution_plan, use_container_width=True, hide_index=True)

        st.success(execution_summary_text(execution_plan))

        st.markdown("### CommSec-Ready Checklist")
        commsec_plan = build_commsec_ready_plan(execution_plan, brokerage_buffer=brokerage_buffer)
        st.dataframe(commsec_plan, use_container_width=True, hide_index=True)

        st.markdown("### Execution Discipline")
        st.info(
            "This is not an instruction to trade immediately. Use it when your cash balance reaches your chosen deployment threshold, "
            "verify current ETF price/spread, and avoid acting on weak or noisy signals."
        )

    st.markdown("### What Could Block Execution?")
    limits = ranking[ranking["Signal"] == "LIMIT"]
    if not limits.empty:
        st.warning("Some companies are already concentration risks. Avoid adding ETFs that mainly increase these exposures unless the ETF demand is very strong.")
        st.dataframe(
            limits[["Ticker", "Company", "Layer", "Exposure", "Reason"]],
            use_container_width=True,
            hide_index=True
        )
    else:
        st.success("No major LIMIT signals detected.")
