...
PROVIDER_URLS = PROVIDER_PAGES


st.set_page_config(
    page_title="AI Portfolio Decision Engine + Monte Carlo Backtest",
    page_icon="⚡",
    layout="wide"
)
...
def run_monte_carlo(start_value, monthly_contribution, months, baseline_weights, test_weights, ai_cycle_strength, volatility_multiplier, simulations, starting_seed):
    rows = []
    for i in range(simulations):
        seed = starting_seed + i
        bt = run_backtest(start_value, monthly_contribution, months, baseline_weights, test_weights, ai_cycle_strength, volatility_multiplier, seed)
        baseline = bt[bt["Strategy"] == "Baseline"].sort_values("Month")
        test = bt[bt["Strategy"] == "Test Portfolio"].sort_values("Month")
        baseline_final = baseline["Portfolio Value"].iloc[-1]
        test_final = test["Portfolio Value"].iloc[-1]
        rows.append({
            "Simulation": i + 1,
            "Seed": seed,
            "Baseline Final": baseline_final,
            "Test Final": test_final,
            "Outperformance $": test_final - baseline_final,
            "Outperformance %": ((test_final / baseline_final) - 1) * 100 if baseline_final else 0,
            "Baseline Max Drawdown %": max_drawdown(baseline["Portfolio Value"]) * 100,
            "Test Max Drawdown %": max_drawdown(test["Portfolio Value"]) * 100,
        })
    return pd.DataFrame(rows)
...
st.caption("Now with Monte Carlo backtesting and scenario guidance.")

tabs = st.tabs([
    "Signal Filtering Engine", "Decision Engine", "Backtest Engine",
    "Dynamic Importance Ranking", "Company Gap Engine", "Overlap Engine",
    "ETF Optimizer", "Holdings Engine", "Auto Holdings Ingestion", "Methodology"
])

with tabs[8]:
    st.subheader("Auto ETF Holdings Ingestion")
    ...
    st.markdown("### Direct holdings file URLs")
    url_inputs = {}
    url_cols = st.columns(2)
    for i, etf in enumerate(["IVV", "VEU", "NDQ", "VHY", "AINF", "SEMI", "CRYP", "GXAI"]):
        with url_cols[i % 2]:
            url_inputs[etf] = st.text_input(f"{etf} direct CSV/XLS/XLSX URL", value="", key=f"holdings_url_{etf}")

    fetch_clicked = st.button("Fetch holdings from URLs")
    fetched_frames = []
    fetch_log = []

    if fetch_clicked:
        for etf, url in url_inputs.items():
            if url.strip():
                try:
                    fetched = fetch_holdings_from_url(url.strip(), etf)
                    fetched_frames.append(fetched)
                    fetch_log.append({"ETF": etf, "Status": "Success", "Rows": len(fetched), "Message": "Fetched and standardized"})
                except Exception as e:
                    fetch_log.append({"ETF": etf, "Status": "Failed", "Rows": 0, "Message": str(e)[:180]})
        if fetched_frames:
            st.session_state["auto_holdings"] = pd.concat(fetched_frames, ignore_index=True)
        else:
            st.session_state.pop("auto_holdings", None)
        st.dataframe(pd.DataFrame(fetch_log), use_container_width=True, hide_index=True)

    st.markdown("### Manual holdings upload")
    uploaded_files = st.file_uploader(
        "Upload one or more holdings files. Required or detectable columns: ETF, Holding Ticker, Company, Weight",
        type=["csv", "xlsx", "xls"],
    ...
    if uploaded_files:
        for file in uploaded_files:
            try:
                name = file.name
                guessed_etf = name.split(".")[0].split("_")[0].split("-")[0].upper()
                if name.lower().endswith((".xlsx", ".xls")):
                    raw = pd.read_excel(file)
                else:
                    raw = pd.read_csv(file)
                standardized = standardize_holdings(raw, default_etf=guessed_etf)
                uploaded_frames.append(standardized)
                upload_log.append({"File": name, "ETF Guess": guessed_etf, "Status": "Success", "Rows": len(standardized)})
            except Exception as e:
                upload_log.append({"File": file.name, "ETF Guess": "", "Status": "Failed", "Rows": 0, "Message": str(e)[:180]})
        if uploaded_frames:
            st.session_state["uploaded_holdings"] = pd.concat(uploaded_frames, ignore_index=True)
        if upload_log:
            st.dataframe(pd.DataFrame(upload_log), use_container_width=True, hide_index=True)
    ...
    active_preview = st.session_state.get("uploaded_holdings", st.session_state.get("auto_holdings", starter_holdings))
    source_name = "Uploaded holdings" if "uploaded_holdings" in st.session_state else ("Auto-fetched holdings" if "auto_holdings" in st.session_state else "Starter holdings")
    st.write(f"Current source: **{source_name}**")
    
def summarize_ingestion_result(df):
    if df is None or len(df) == 0:
        return pd.DataFrame([{"Metric": "Rows", "Value": 0}])
    summary = []
    summary.append({"Metric": "Rows", "Value": len(df)})
    summary.append({"Metric": "Columns", "Value": len(df.columns)})
    if "ETF" in df.columns:
        summary.append({"Metric": "ETFs detected", "Value": df["ETF"].nunique()})
        summary.append({"Metric": "ETF list", "Value": ", ".join(sorted(df["ETF"].dropna().astype(str).unique()))})
...
st.dataframe(summarize_ingestion_result(active_preview), use_container_width=True, hide_index=True)

    if st.button("Clear uploaded / fetched holdings and revert to starter data"):
        st.session_state.pop("uploaded_holdings", None)
        st.session_state.pop("auto_holdings", None)
        st.rerun()

holdings = st.session_state.get("uploaded_holdings", st.session_state.get("auto_holdings", starter_holdings.copy()))
...
with tabs[2]:
    st.subheader("Monte Carlo + Adjustable Backtest Engine")
    st.write("Compare any baseline portfolio against any test portfolio. Single-run shows one path; Monte Carlo tests many paths.")

    with st.expander("How to use these controls", expanded=True):
        st.markdown("""
**Scenario seed:** controls the random market path. Use one seed for a single example, or Monte Carlo to run many seeds.  
**AI cycle strength:** 0.0 = no AI boom, 1.0 = base case, 2.0 = strong AI cycle, 3.0 = extreme AI supercycle.  
**Volatility multiplier:** 0.5 = calm, 1.0 = normal, 1.5 = choppy, 2.5+ = crisis/mania.  
**Monte Carlo simulations:** 50–100 is quick, 250+ is more robust.
""")
    ...
    with b4:
        random_seed = st.number_input("Scenario seed", min_value=1, value=7, step=1, help="Try 7, 21, 42, 99. Different seeds create different simulated market paths.")

    s1, s2, s3 = st.columns(3)
    with s1:
        ai_cycle_strength = st.slider("AI cycle strength", 0.0, 3.0, 1.0, 0.1, help="0=no AI tailwind, 1=base case, 2=strong AI boom, 3=extreme supercycle.")
    with s2:
        volatility_multiplier = st.slider("Volatility multiplier", 0.25, 3.0, 1.0, 0.05, help="0.5=calm, 1=normal, 1.5=choppy, 2.5+=chaos.")
    with s3:
        monte_carlo_runs = st.slider("Monte Carlo simulations", 10, 500, 100, 10, help="Runs many different seeds to estimate probability of outperformance.")
    ...
    st.markdown("## Single Scenario Path")
    ...
    if delta > 0:
        st.success(f"In this single scenario, the test portfolio ends ${delta:,.0f} ahead.")
    else:
        st.warning(f"In this single scenario, the test portfolio ends ${abs(delta):,.0f} behind.")
    ...
    avg_outperformance = mc["Outperformance $"].mean()
    median_outperformance = mc["Outperformance $"].median()
    p10 = mc["Outperformance $"].quantile(0.10)
    p90 = mc["Outperformance $"].quantile(0.90)
    avg_dd_delta = mc["Test Max Drawdown %"].mean() - mc["Baseline Max Drawdown %"].mean()

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Win rate", f"{win_rate:.1f}%")
    m2.metric("Avg outperformance", f"${avg_outperformance:,.0f}")
    m3.metric("Median outperformance", f"${median_outperformance:,.0f}")
    m4.metric("Avg drawdown difference", f"{avg_dd_delta:.1f}%")

    m5, m6 = st.columns(2)
    m5.metric("10th percentile outcome", f"${p10:,.0f}")
    m6.metric("90th percentile outcome", f"${p90:,.0f}")

    if win_rate >= 65 and avg_outperformance > 0:
        st.success("Monte Carlo result: the test portfolio shows robust outperformance in this scenario set.")
    elif win_rate >= 50 and avg_outperformance > 0:
        st.info("Monte Carlo result: the test portfolio has a positive edge, but not overwhelming.")
    else:
        st.warning("Monte Carlo result: the test portfolio does not show a reliable edge under these assumptions.")

    st.markdown("### Monte Carlo Distribution")
    hist = pd.cut(mc["Outperformance $"], bins=20).value_counts().sort_index().reset_index()
    hist.columns = ["Outperformance Bucket", "Count"]
    hist["Outperformance Bucket"] = hist["Outperformance Bucket"].astype(str)
    st.bar_chart(hist.set_index("Outperformance Bucket")["Count"])

    st.markdown("### Simulation Detail")
    st.dataframe(mc.round(1), use_container_width=True, hide_index=True)

    st.caption("This is synthetic scenario testing, not true historical performance. Use it to stress-test assumptions, not to forecast returns.")
...
Monte Carlo runs many simulated market paths using different scenario seeds. This gives a probability-style view rather than relying on one lucky or unlucky run.

Key outputs:

- **Win rate**: how often the test portfolio beats the baseline.
- **Average outperformance**: average dollar difference across simulations.
- **Median outperformance**: middle result, less skewed by extreme outcomes.
- **10th percentile**: bad-case result.
- **90th percentile**: strong-case result.
- **Drawdown difference**: whether the test portfolio is taking more downside risk.

### Scenario Controls

- **Scenario seed**: controls one random path. For Monte Carlo, it becomes the starting seed.
- **AI cycle strength**:
  - 0.0 = no AI boom
  - 1.0 = base case
  - 2.0 = strong AI cycle
  - 3.0 = extreme AI supercycle
...
### Auto Holdings Ingestion

The app can ingest direct provider CSV/XLS/XLSX holdings URLs or manually uploaded holdings files. Provider pages are included because direct download links often change. Uploaded holdings override auto-fetched holdings, and both override starter holdings.

### Important

This is synthetic scenario testing, not true historical performance. It is useful for stress-testing portfolio logic, not forecasting returns.
